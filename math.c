...
uint32_t getBytesForAdpcmSamples(uint32_t samples){
	...
	const uint32_t frames = samples / SAMPLES_PER_FRAME;
	const uint32_t extraSamples = samples % SAMPLES_PER_FRAME;
...
uint32_t getNibblesForNSamples(uint32_t samples){
	const uint32_t frames = samples / SAMPLES_PER_FRAME;
	const uint32_t extraSamples = samples % SAMPLES_PER_FRAME;
	const uint32_t extraNibbles = extraSamples == 0 ? 0 : extraSamples + 2;

	return NIBBLES_PER_FRAME * frames + extraNibbles;
}

uint32_t getSampleForAdpcmNibble(uint32_t nibble){
	const uint32_t frames = nibble / NIBBLES_PER_FRAME;
	const uint32_t extraNibbles = nibble % NIBBLES_PER_FRAME;
	const uint32_t samples = SAMPLES_PER_FRAME * frames;
