...
void encode(int16_t* src, uint8_t* dst, ADPCMINFO* cxt, uint32_t samples){
	...
	const uint32_t frameCount = samples / SAMPLES_PER_FRAME + (samples % SAMPLES_PER_FRAME != 0);
	...
	for (int i = 0; i < frameCount; ++i, pcm += SAMPLES_PER_FRAME, adpcm += BYTES_PER_FRAME) {
		const uint32_t sampleCount = MIN(samples - i * SAMPLES_PER_FRAME, SAMPLES_PER_FRAME);
...
bool AnalyzeRanges(tvec mtx[3], int* vecIdxsOut){
	...
	for (int i = 1; i <= 2; i++) {
		...
		if (maxIndex != i) {
			for (int y = 1; y <= 2; y++) {
				const double tmp = mtx[maxIndex][y];
...
double ContrastVectors(const tvec source1, const tvec source2){
	const double val = (source2[2] * source2[1] + -source2[1]) / (1.0 - source2[2] * source2[2]);
	const double val1 = (source1[0] * source1[0]) + (source1[1] * source1[1]) + (source1[2] * source1[2]);
...
void FilterRecords(tvec vecBest[8], int exp, tvec records[], int recordCount){
	...
	for (int x = 0; x < 2; x++) {
		...
		for (int z = 0; z < recordCount; z++) {
			...
			double value = 1.0e30;
			for (int i = 0; i < exp; i++) {
				const double tempVal = ContrastVectors(vecBest[i], records[z]);
				if (tempVal < value) {
					value = tempVal;
					index = i;
				}
...
void correlateCoefs(const int16_t* source, uint32_t samples, int16_t* coefsOut){
	const uint32_t numFrames = (samples + 13) / 14;
	uint32_t frameSamples;

	short* blockBuffer = calloc(sizeof(short), 0x3800);
	short pcmHistBuffer[2][14] = {0};

	tvec vec1;
	tvec vec2;

	tvec* records = calloc(sizeof(tvec), numFrames * 2);
	int recordCount = 0;
